module ViewsHelper
  ...
  def errors_chart_series_for(current_object)
    current_period_day = Time.now.strftime("%Y%m%d")
    current_period_id = TimeScale.where("period_day = ?", current_period_day).take.period_id
    measured_children = DmMeasure.where("period_id = ? and ODQ_parent_id = ? and score < 100", current_period_id, current_object.id).where("odq_object_id <> odq_parent_id").select("odq_object_id as id, odq_object_name as name, odq_object_code as code, error_count, odq_object_url as url")
  end
