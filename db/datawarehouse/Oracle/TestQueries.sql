...
select BP.playground_id, BP.playground_id || '-BP-' || BP.id ODQ_object_id, BP.playground_id || '-BF-' || BP.business_flow_id ODQ_parent_id, to_char(current_date, 'YYYYMMDD') Period_id,  
BP.name BP, 
count(record_id) all_records, 
sum(case
when  odq_app.bitand2(power(2,BR.id), rawtohex(error_mask)) <> 0 then 1
...
inner join odq_app.business_rules BR on BR.business_process_id = BP.id
inner join odq_app.dwh_records DWH on odq_app.bitand2(power(2,BR.id), rawtohex(rule_mask)) <> 0
group by BP.playground_id, BP.playground_id || '-BP-' || BP.id, BP.playground_id || '-BF-' || BP.business_flow_id, to_char(current_date, 'YYYYMMDD'),  
BP.name
order by 2



select BF.playground_id, BF.playground_id || '-BF-' || BF.id ODQ_object_id, BF.playground_id || '-BA-' || BF.business_area_id ODQ_parent_id, to_char(current_date, 'YYYYMMDD') Period_id,  
BF.name BF, 
count(record_id) all_records, 
sum(case
when  odq_app.bitand2(power(2,BR.id), rawtohex(error_mask)) <> 0 then 1
...
inner join odq_app.business_rules BR on BR.business_process_id = BP.id
inner join odq_app.dwh_records DWH on odq_app.bitand2(power(2,BR.id), rawtohex(rule_mask)) <> 0
group by BF.playground_id, BF.playground_id || '-BF-' || BF.id, BF.playground_id || '-BA-' || BF.business_area_id, to_char(current_date, 'YYYYMMDD'),  
BF.name 
order by 2



select BA.playground_id, BA.playground_id || '-BA-' || BA.id ODQ_object_id, BA.playground_id ODQ_parent_id, to_char(current_date, 'YYYYMMDD') Period_id,  
BA.name BA, 
count(record_id) all_records, 
sum(case
when  odq_app.bitand2(power(2,BR.id), rawtohex(error_mask)) <> 0 then 1
...
inner join odq_app.business_rules BR on BR.business_process_id = BP.id
inner join odq_app.dwh_records DWH on odq_app.bitand2(power(2,BR.id), rawtohex(rule_mask)) <> 0
group by BA.playground_id, BA.playground_id || '-BA-' || BA.id, BA.playground_id, to_char(current_date, 'YYYYMMDD'),  
BA.name
order by 2

/*UNION of countings for several aggregation levels*/

select BR.playground_id, to_char(BR.playground_id) || '-BR-' || to_char(BR.id) ODQ_object_id, to_char(BR.playground_id) || '-BP-' || to_char(BR.business_process_id) ODQ_parent_id, to_char(current_date, 'YYYYMMDD') Period_id,  
BR.name , 
count(record_id) all_records, 
sum(case
when  odq_app.bitand2(power(2,BR.id), rawtohex(error_mask)) <> 0 then 1
...
select BP.playground_id, to_char(BP.playground_id) || '-BP-' || to_char(BP.id) ODQ_object_id, to_char(BP.playground_id) || '-BF-' || to_char(BP.business_flow_id) ODQ_parent_id, to_char(current_date, 'YYYYMMDD') Period_id,  
BP.name , 
count(record_id) all_records, 
sum(case
when  odq_app.bitand2(power(2,BR.id), rawtohex(error_mask)) <> 0 then 1
...
select BF.playground_id, to_char(BF.playground_id) || '-BF-' || to_char(BF.id) ODQ_object_id, to_char(BF.playground_id) || '-BA-' || to_char(BF.business_area_id) ODQ_parent_id, to_char(current_date, 'YYYYMMDD') Period_id,  
BF.name , 
count(record_id) all_records, 
sum(case
when  odq_app.bitand2(power(2,BR.id), rawtohex(error_mask)) <> 0 then 1
...
select BA.playground_id, to_char(BA.playground_id) || '-BA-' || to_char(BA.id) ODQ_object_id, to_char(BA.playground_id) ODQ_parent_id, to_char(current_date, 'YYYYMMDD') Period_id,  
BA.name , 
count(record_id) all_records, 
sum(case
when  odq_app.bitand2(power(2,BR.id), rawtohex(error_mask)) <> 0 then 1
...
/* Insertion of the results of counting */

insert into odq_app.dm_measures(PLAYGROUND_ID,ODQ_OBJECT_ID,ODQ_PARENT_ID,PERIOD_ID,ALL_RECORDS,BAD_RECORDS,SCORE,CREATED_BY,UPDATED_BY,CREATED_AT,UPDATED_AT,PROCESS_ID) 
select PLAYGROUND_ID,ODQ_OBJECT_ID,ODQ_PARENT_ID,PERIOD_ID,ALL_RECORDS,BAD_RECORDS,SCORE,'ODQ','ODQ',current_timestamp, current_timestamp,0 from (
...
/* Assign -1 for rules with no record evaluated */
select BR.playground_id, to_char(BR.playground_id) || '-BR-' || to_char(BR.id) ODQ_object_id, to_char(BR.playground_id) || '-BP-' || to_char(BR.business_process_id) ODQ_parent_id, to_char(current_date, 'YYYYMMDD') Period_id,  
BR.name , 
count(record_id) all_records, 
sum(case
...
insert into odq_app.dm_measures(PLAYGROUND_ID,ODQ_OBJECT_ID,ODQ_PARENT_ID,PERIOD_ID,ALL_RECORDS,BAD_RECORDS,SCORE,CREATED_BY,UPDATED_BY,CREATED_AT,UPDATED_AT,PROCESS_ID) 
select PLAYGROUND_ID,ODQ_OBJECT_ID,ODQ_PARENT_ID,PERIOD_ID,ALL_RECORDS,BAD_RECORDS,SCORE,'Rake','Rake',current_timestamp, current_timestamp,0 from (
