...
insert into odq_dwh.dm_processes(PLAYGROUND_ID,ODQ_OBJECT_ID,ODQ_PARENT_ID,ODQ_OBJECT_NAME, ODQ_OBJECT_CODE, ODQ_OBJECT_URL, PERIOD_DAY, ALL_RECORDS,ERROR_COUNT,SCORE,WORKLOAD,ADDED_VALUE,MAINTENANCE_COST,PERIOD_ID,CREATED_BY,UPDATED_BY,CREATED_AT,UPDATED_AT, TERRITORY_ID, ORGANISATION_ID) 
select DWH.PLAYGROUND_ID, DWH.ID, DWH.PARENT_ID, DWH.NAME, DWH.CODE, DWH.URL, DTIME.PERIOD_DAY, DWH.ALL_RECORDS, DWH.ERROR_COUNT, DWH.SCORE, DWH.WORKLOAD, DWH.ADDED_VALUE, DWH.MAINTENANCE_COST, DTIME.PERIOD_ID,'Rake','Rake',current_timestamp, current_timestamp, 0, 0 from (
select BR.playground_id||'0', BR.id||'7', BR.business_process_id||'6' parent_id, BR.name, BR.code, '/business_rules/'||BR.id url, to_char(current_date, 'YYYYMMDD') Period_day,  
br.all_records, 
br.bad_records error_count,
0 score,
0 workload,
...
0 maintenance_cost
from odq_app.business_rules BR 
group by BR.playground_id||'0', BR.id||'7', BR.business_process_id||'6', BR.name, BR.code, '/business_rules/'||BR.id,  to_char(current_date, 'YYYYMMDD') 
UNION
select BP.playground_id||'0', BP.id||'6', BP.business_flow_id||'5' parent_id, BP.name, BP.code, '/business_processes/'||BP.id url, to_char(current_date, 'YYYYMMDD') Period_day, 
bp.all_records, 
bp.bad_records  error_count,
0 score,
0 workload,
...
from odq_app.business_processes BP 
inner join odq_app.business_rules BR on BR.business_process_id = BP.id
group by BP.playground_id||'0',  BP.id||'6', BP.business_flow_id||'5', BP.name, BP.code, '/business_processes/'||BP.id, to_char(current_date, 'YYYYMMDD')
UNION
select BF.playground_id||'0', BF.id||'5', BF.business_area_id||'4' parent_id, BF.name, BF.code, '/business_flows/'||BF.id url, to_char(current_date, 'YYYYMMDD') Period_day,  
bf.all_records, 
bf.bad_records  error_count,
0 score,
0 workload,
...
inner join odq_app.business_processes BP on BP.business_flow_id = BF.id
inner join odq_app.business_rules BR on BR.business_process_id = BP.id
group by BF.playground_id||'0', BF.id||'5', BF.business_area_id||'4', BF.name, BF.code, '/business_flows/'||BF.id, to_char(current_date, 'YYYYMMDD')
UNION
select BA.playground_id||'0', BA.id||'4', BA.playground_id||'0' parent_id, BA.name, BA.code, '/business_areas/'||BA.id url, to_char(current_date, 'YYYYMMDD') Period_day,
ba.all_records, 
ba.bad_records  error_count,
0 score,
0 workload,
...
inner join odq_app.business_processes BP on BP.business_flow_id = BF.id
inner join odq_app.business_rules BR on BR.business_process_id = BP.id
group by BA.playground_id||'0',  BA.id||'4', BA.playground_id||'0', BA.name, BA.code, '/business_areas/'||BA.id, to_char(current_date, 'YYYYMMDD')
UNION
select PG.id||'0', PG.id||'0', PG.id||'0' parent_id, PG.name, PG.code, '/playgrounds/'||PG.id url, to_char(current_date, 'YYYYMMDD') Period_day,
pg.all_records, 
pg.bad_records  error_count,
0 score,
0 workload,
...
inner join odq_app.business_processes BP on BP.business_flow_id = BF.id
inner join odq_app.business_rules BR on BR.business_process_id = BP.id
group by PG.id||'0', PG.id||'0', PG.id||'0', PG.name, PG.code, '/playgrounds/'||PG.id, to_char(current_date, 'YYYYMMDD')
)  DWH
inner join odq_dwh.DIM_TIME DTIME on DTIME.PERIOD_DATE between current_date -20 and current_date
where dwh.playground_id = 99;
